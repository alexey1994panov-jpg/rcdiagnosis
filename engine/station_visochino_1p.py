...
def get_station_model_1p() -> StationModel1P:
    ...
    # два автодействия, оба распространяются на все РЦ
    nas = AutoAction(id="NAS", state=4, rc_ids=list(rc_ids))   # Н Автодействие
    chas = AutoAction(id="CHAS", state=4, rc_ids=list(rc_ids)) # Ч Автодействие

    # Признак замыкания по РЦ:
    rc_has_route_lock = {
        "10-12SP": False,
        "1P": True,
        "1-7SP": True,
    ...
    # Светофоры и их привязка к РЦ
    # - Ч1: 1P -> 1-7SP
    # - НМ1: 1P -> 10-12SP
    # - М1: НП -> 1-7SP  (НП пока нет в rc_ids)
    # - ЧМ1: 1АП -> 10-12SP (1АП пока нет в rc_ids)
    signal_ids = ["Ч1", "НМ1", "ЧМ1", "М1"]
    signal_prev_by_id = {
        "Ч1": "1P",
        "НМ1": "1P",
        "ЧМ1": "1АП",
