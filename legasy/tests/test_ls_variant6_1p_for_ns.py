...
def test_ls_variant6_simple_scenario_from_json(tmp_path: Path):
    """
    Сценарий для ЛС6:

    1) 3 c: все РЦ замкнуты (4), М1 открыт (4), контролируемая 1-7SP ещё свободна.
    2) 3 c: 10-12SP и 1P заняты (7), 1-7SP свободна (4) — формируется ЛС6.
    3) 3 c: 10-12SP, 1P и 1-7SP заняты (7) — контролируемая 1-7SP занята, ЛС6 закрывается.
    """

    scenario_json = {
        "options": {
            # ЛЗ — все выкл, чтобы не мешали
            "t_s0101": 0,
            "t_lz01": 0,
            "t_kon_v1": 0,
            "t_pause_v1": 0,
            ...
            "t_pause_v13": 0,
            "enable_v13": False,
            # ЛС1/2/4/5/9 — выключены по таймингам
            "t_c0101_ls": 0,
            "t_ls01": 0,
            "t_kon_ls1": 0,
            "t_pause_ls1": 0,
            ...
            "t_pause_ls9": 0,
            "enable_ls9": True,
            # ЛС6 — включена и с ненулевыми таймингами
            "t_s0106_ls": 3,
            "t_ls06": 3,
            "t_kon_ls6": 3,
            "t_pause_ls6": 0,
        ...
        "steps": [
            {
                # шаг 1: все РЦ замкнуты, М1 открыт
                "t": 3,
                "rc_states": {
                    "1P": 4,
                    "1-7SP": 4,
            ...
            {
                # шаг 2: 10-12SP и 1P заняты, 1-7SP ещё свободна — формируется ЛС6
                "t": 3,
                "rc_states": {
                    "1P": 7,
                    "1-7SP": 7,
            ...
            {
                # шаг 3: все три РЦ заняты, контролируемая 1-7SP занята — ЛС6 должен закрыться
                "t": 3,
                "rc_states": {
                    "1P": 7,
                    "1-7SP": 7,
    ...
    # ЛС6 должен открыться
    assert "lls_v6_open" in flags

    # ЛС6 должен быть активен хотя бы на одном шаге
    assert "lls_v6" in flags

    # К моменту, когда 1-7SP занята, ЛС6 должен закрыться
    assert "lls_v6_closed" in flags
