# tests/test_variant13_10_12sp_from_json.py
...
def _build_steps_from_json(data: dict) -> tuple[dict, list[ScenarioStep]]:
    ...
    # v13_ctrl_rc_id теперь игнорируется бэком, но в JSON может остаться
    options.pop("v13_ctrl_rc_id", None)
...
def test_variant13_scenario_for_10_12sp():
    """
    Вариант 13 с контролируемой 10-12SP, смежная 1P, сигнал НМ1 закрыт.

    Фазы:
      1) 10-12SP 3 (ctrl_free), 1P 4 (adj_free+locked), НМ1 15 — S0113
      2) 10-12SP 3 (ctrl_free), 1P 7 (adj_occ+locked), НМ1 15 — S0213
      3) 10-12SP 6 (ctrl_occ), 1P 7 (adj_occ+locked), НМ1 15 — LZ13 (открытие)
      4) 10-12SP 3 (ctrl_free), 1P 3 (adj_free), НМ1 15 — завершение
    """
    ...
    scenario_json = {
        ...
        "options": {
            ...
            "t_kon_v13": 3.0,
            "enable_v13": True,
            # поле осталось от фронта, но бэком игнорируется
            "v13_ctrl_rc_id": "10-12SP",

            # вырубаем всё остальное
            "enable_v1": False,
            "enable_v2": False,
            "enable_v3": False,
            "enable_v4": False,
        ...
        "steps": [
            {
                # фаза 01: ctrl_free, adj_free+locked, НМ1 закрыт
                "t": 3.0,
            ...
            {
                # фаза 02: ctrl_free, adj_occ+locked, НМ1 закрыт
                "t": 3.0,
            ...
            {
                # LZ13: ctrl_occ, adj_occ+locked, НМ1 закрыт
                "t": 3.0,
                "rc_states": {"10-12SP": 6, "1P": 7, "1-7SP": 3},
                "switch_states": {"Sw10": 3, "Sw1": 3, "Sw5": 3},
                "modes": {},
                "mu": {"10-12SP": 3, "1P": 3, "1-7SP": 3},
                "dispatcher_control_state": 4,
    ...
    print("\n=== TIMELINE v13 ctrl=10-12SP ===")
    for i, st in enumerate(timeline):
        print(
            f"Step {i}: t={st.t}, variant={st.variant}, "
            f"lz={st.lz_state}, flags={st.flags}, "
            f"rc_states={st.rc_states}"
        )

    # v13 должен участвовать
    ...
    # хотя бы один шаг с variant == 13
    assert 13 in [st.variant for st in timeline]
    # проверяем открытие и закрытие
