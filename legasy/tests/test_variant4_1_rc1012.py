...
from engine.station_visochino_1p import get_station_model_1012_ctrl
...
def make_step(t, rc_1012, rc_1p, rc_17, chm1_state=15, m1_state=3):
    """
    Шаг сценария для ветки 4.1:
    - контролируемая 10-12SP,
    - входной поездной ЧМ1 (prev->ctrl),
    - маневровый М1 присутствует, но для 4.1 не критичен.
    """
    return ScenarioStep(
        t=float(t),
        rc_states={"10-12SP": rc_1012, "1P": rc_1p, "1-7SP": rc_17},
        switch_states={"Sw10": 3, "Sw1": 3, "Sw5": 3},
        signal_states={"Ч1": 0, "НМ1": 0, "ЧМ1": chm1_state, "М1": m1_state},
        modes={},
...
def test_variant4_for_1012sp(station):
    # включаем только v4, остальные ЛЗ/ЛС отключаем
    opts = {
        "enable_v1": False,
        "enable_v2": False,
        "enable_v3": False,
    ...
    steps = [
        # ДАНО: prev NC, curr свободна, next свободна, ЧМ1 закрыт, 3 сек
        make_step(3.0, rc_1012=3, rc_1p=3, rc_17=3, chm1_state=15, m1_state=3),
        # КОГДА: curr занята, ЧМ1 закрыт, 3 сек
        make_step(3.0, rc_1012=6, rc_1p=3, rc_17=3, chm1_state=15, m1_state=3),
