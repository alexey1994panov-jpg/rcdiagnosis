...
def make_step(t, rc_1012, rc_1p):
    return ScenarioStep(
        t=float(t),
        rc_states={"10-12SP": rc_1012, "1P": rc_1p, "1-7SP": 3},
...
def test_ls_variant2_from_json(station):
    # Опции из JSON
    opts = {
        "t_s0101": 3.0,
        "t_lz01": 3.0,
    ...
    # Шаги из JSON: предыдущая РЦ и контролируемая заняты, затем ctrl свободна, затем снова заняты
    steps = [
        make_step(1, rc_1012=6, rc_1p=6),
        make_step(1, rc_1012=6, rc_1p=6),
        make_step(1, rc_1012=6, rc_1p=6),
        make_step(1, rc_1012=6, rc_1p=3),
        make_step(1, rc_1012=6, rc_1p=3),
        make_step(1, rc_1012=6, rc_1p=6),
        make_step(1, rc_1012=6, rc_1p=6),
        make_step(1, rc_1012=6, rc_1p=6),
    ...
    assert any("lls_v2_open" in s.flags for s in timeline), "Нет lls_v2_open"
    assert any("lls_v2" in s.flags for s in timeline), "Нет активного lls_v2"
    # закрытие может не успеть случиться в этом коротком сценарии, поэтому не проверяем
