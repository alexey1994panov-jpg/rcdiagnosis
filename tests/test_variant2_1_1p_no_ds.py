...
def test_variant2_no_ds_when_initial_prev_not_adjacent(station):
    opts_dict = {
        "t_s0101": 3.0,
        "t_lz01": 3.0,
        "t_kon_v1": 3.0,
    ...
    steps = [
        ScenarioStep(
            t=1.0,
            rc_states={"10-12SP": 3, "1P": 3, "1-7SP": 6},
    ...
    # Вариант 2 не должен формироваться
    assert not any("llz_v2_open" in s.flags for s in timeline), "В2 не должен открываться"
    assert not any("llz_v2" in s.flags and s.variant == 2 for s in timeline), "variant=2 недопустим"
    assert not any("llz_v2_closed" in s.flags for s in timeline), "В2 не должен закрываться"
