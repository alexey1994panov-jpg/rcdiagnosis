# -*- coding: utf-8 -*-
# build_signals_logic.py — все светофоры (маневровые + путевые/поздние)
...
def get_sec_name(sec_name: str) -> str | None:
    """
    sec_name -> имя секции (РЦ), если есть такой узел с type == 1 и name == sec_name.
    Возвращает sec_name, иначе None.
    """
...
for node_id, node_data in NODES.items():
    ...
    # интересуют только светофоры
    if node_type not in (3, 4):
        continue

    # здесь лежат ИМЕНА РЦ, а не ID
    ...
    # берём первую валидную секцию из prev_links
    ...
    # берём первую валидную секцию из next_links
...
if __name__ == "__main__":
    ...
    print("=== SIGNALS_LOGIC (маневровые + путевые) ===")
    ...
    print(f"\nСохранено: {out_path}")
    print(f"\nНайдено светофоров: {len(SIGNALS_LOGIC)}")
