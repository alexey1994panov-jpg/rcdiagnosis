# Обновлённый sim_core.py с обратной совместимостью
...
class SimulationContext:
    ...
    def _compute_effective_neighbors_with_control(
        self,
        ctrl_rc_id: str,  # NEW: параметр — для какой РЦ вычисляем
        dt: float,
    ) -> Tuple[Optional[str], Optional[str], bool, bool]:
        """
        Вычисляет соседей для конкретной контролируемой РЦ.
        
        Для C: функция принимает rc_id, возвращает структуру с соседями.
        """
